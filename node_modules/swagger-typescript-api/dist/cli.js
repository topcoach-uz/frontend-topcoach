...
${u}${d}`).join("")}).join(`
`),V=(e,t)=>{let{options:n,maxLength:s}=A(e[c].options),{commands:r,maxLength:i}=P.filter(e,l=>l.name!==c).reduce((l,p)=>{let g=A(p.options),$=`${p.name}${g.length?" [options]":""}`,m=p.description,f=Math.max($.length,g.maxLength);return f>l.maxLength&&(l.maxLength=f),l.commands.push({description:m,name:$,options:g}),l},{commands:[],maxLength:s}),u=C(n,s,2),a=r.map(l=>{let p=`  ${l.name.padEnd(i," ")}    `,g=Array(p.length).fill(" ").join(""),$=l.description.split(`
